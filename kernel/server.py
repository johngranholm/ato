...
Token + Origin/Host allowlist. Conversational endpoints.
"""
import json
import queue
import secrets
...
HOOKS = {"greet": None, "handle_message": None, "tts": None, "list_glbs": None,
         "read_glb": None, "info_extra": None, "models": None}
_greeted = {"done": False}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p == "/":
            self._send(200, "text/html; charset=utf-8",
                       _load_ui().replace("__SESSION_TOKEN__", SESSION_TOKEN).encode())
        elif p == "/ping":
            ...
            self._json({"model": config.MODEL, "mode": busmod.state["mode"],
                        ...
                        "tts": bool(config.TTS_ENABLED), "awaiting": busmod.state["awaiting"],
                        "sandbox": sandbox.status(), **extra})
        elif p == "/conversation":
            self._json({"messages": conv.recent_for_ui()})
        elif p == "/backups":
            ...
            self._send(404, "text/plain", b"no glb") if data is None \
                else self._send(200, "model/gltf-binary", data)
        elif p == "/models":
            self._json(HOOKS["models"]() if HOOKS["models"] else {"models": ""})
        elif p == "/hello":
            ...
            if (parse_qs(urlparse(self.path).query).get("t") or [""])[0] != SESSION_TOKEN:
    ...
    def do_POST(self):
        ...
        if p in ("/message", "/goal"):
            text = (b.get("text") or b.get("goal") or "").strip()
            if text:
                threading.Thread(target=HOOKS["handle_message"], args=(text,), daemon=True).start()
            self._json({"ok": True})
        elif p == "/approve":
            ...
            busmod.do_reset("manual reset"); self._json({"ok": True})
        elif p == "/new":
            conv.clear(); busmod.bus.emit("system", "Started a fresh conversation.")
            self._json({"ok": True})
        elif p == "/rollback":
            ok, msg = selfedit.rollback((b.get("to") or "").strip() or None)
            if ok:
                busmod.bus.emit("system", "Rollback: " + msg); busmod.schedule_reboot("rollback")
            else:
            ...
            if m in ("step", "auto"):
                busmod.state["mode"] = m
                busmod.bus.emit("system", f"Mode: {m.upper()}."); busmod.set_status()
            self._json({"ok": True, "mode": busmod.state["mode"]})
        elif p == "/memory/toggle":
            mem.MEM["memory_on"] = not mem.MEM.get("memory_on", True); mem.save_mem()
            ...
            if HOOKS["tts"]:
                code, ctype, payload = HOOKS["tts"](b); self._send(code, ctype, payload)
            else:
